...
| `config.upstreams.<id>.dnsHook.propagation.enabled` | `true` | Enable the authoritative dns-01 propagation check (background; non-fatal on timeout — triggers upstream anyway) |
| `config.upstreams.<id>.dnsHook.propagation.timeoutSeconds` | `300` | Max wait for authoritative TXT convergence before triggering upstream regardless |
...
| `config.upstreams.<id>.dnsHook.propagation.quorumPercent` | `100` | Required percentage of authoritative nameservers to be stable. Lower (e.g. `80`) for CAs whose anycast NS converge unevenly |
